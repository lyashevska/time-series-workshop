...
The source code for the package is available at: <https://github.com/seananderson/glmmfields>. It's been written by myself and Eric Ward. You can install it with this code:
...
Let's fit the model. For maximum simplicity, we don't have any covariates here and we are using a Gaussian error structure with an identity link (i.e. similar to a linear regression). The package can work with most of the same families as `glm()`.

...
## plot the data
plot(year, y, type = "o", col = "purple",  pch = 19, bty = "l")
...
plot(year, y, type = "o", col = "darkgrey",  pch = 19, bty = "l", main = "Sequential processing changepoints")
for(i in 1:3){
  idx <- (k[i] + 1):k[i + 1]
...
plot(year2, y, type = "o", col = "darkgrey", ylab = ylab, pch = 19, bty = "l", main = "Sequential processing changepoints")
for(i in 1:3){
  idx <- (k[i] + 1):k[i + 1]
  points(year[idx], y[idx], pch = 19, col = cols[i])
  curve(mean(y[idx]) + 0 * x, from = year[k[i] + 1], to = year[k[i+1]], add = TRUE, col = cols[i])

...
An alternative prior, that we have used in our more recent work is `gamma(shape
= 2, rate = 0.1)` as recommended in Juarez, M. A. and M. F. J. Steel. 2010.
Model-based clustering of non-Gaussian panel data based on skew-t
distributions. J. Bus. Econ. Stat., 28:52–66.

<https://github.com/seananderson/heavy-tails/blob/master/analysis/1-compile-models.R>

```{r}
m <- stan("heavy-tails/gompertz-ss-est.stan", 
  data = list(N = nrow(d), y = log(d$population_untransformed), nu_rate = 0.01, sigma_obs = 0.05),
  iter = 2000, control = list(max_treedepth = 20, adapt_delta = 0.95))

e <- extract(m, pars = "U")
pred <- data.frame(
  est = apply(e$U, 2, median),
  l = apply(e$U, 2, quantile, probs = 0.025),
  h = apply(e$U, 2, quantile, probs = 0.975),
  sample_year = d$sample_year)
ggplot(pred, aes(sample_year, est, ymin = l, ymax = h)) +
  geom_ribbon(alpha = 0.4) +
  geom_line() +
